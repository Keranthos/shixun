...
func main() {
	...
	ragIndexer := service.NewRagIndexer(db, cfg)
	ragHook := service.NewRagIndexHook(ragIndexer)
	toolHandler := handler.NewToolHandler(toolService, ragHook)
	courseHandler := handler.NewCourseHandler(courseService, ragHook)
	projectHandler := handler.NewProjectHandler(projectService, ragHook)
	ragAgentSvc := service.NewRagAgentService(db, cfg)
	adminHandler := handler.NewAdminHandler(adminService, ragHook)
	uploadHandler := handler.NewUploadHandler()
	ragAgentHandler := handler.NewRagAgentHandler(ragAgentSvc, ragIndexer)
	...
	agentChat := r.Group("/api/agent")
	agentChat.Use(middleware.AuthMiddleware(), middleware.AuthRateLimit(cfg.RagRateLimitPerMin, time.Minute))
	{
		agentChat.POST("/rag", ragAgentHandler.Chat) // 登录用户可问答（学生只读）
	}

	agentAdmin := r.Group("/api/agent")
	agentAdmin.Use(middleware.AuthMiddleware(), middleware.AdminMiddleware(), middleware.AuthRateLimit(cfg.RagRateLimitPerMin, time.Minute))
	{
		agentAdmin.GET("/status", ragAgentHandler.Status)
		agentAdmin.POST("/reindex", ragAgentHandler.Reindex)
	}
	...
	if cfg.AgentBaseURL != "" && cfg.AgentAutoReindex {
		go func() {
			time.Sleep(5 * time.Second)
			ctx, cancel := context.WithTimeout(context.Background(), 10*time.Minute)
			defer cancel()
			if err := ragIndexer.EnsureIndexedIfEmpty(ctx); err != nil {
				log.Printf("[RAG] auto reindex: %v", err)
			}
		}()
	}
