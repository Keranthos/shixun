...
type ProjectHandler struct {
	...
	ragHook        *service.RagIndexHook
}

func NewProjectHandler(projectService service.ProjectService, ragHook *service.RagIndexHook) *ProjectHandler {
	return &ProjectHandler{projectService: projectService, ragHook: ragHook}
}
...
func (h *ProjectHandler) UpdateProject(c *gin.Context) {
	...
	if h.ragHook != nil {
		h.ragHook.OnResourceChanged("project", projectID)
	}
...
func (h *ProjectHandler) AddComment(c *gin.Context) {
	...
	if h.ragHook != nil {
		if id := extractCommentID(result); id != "" {
			h.ragHook.OnCommentAdded(id)
		}
	}
...
func (h *ProjectHandler) DeleteComment(c *gin.Context) {
	...
	if h.ragHook != nil {
		h.ragHook.OnCommentDeleted(commentID)
	}
