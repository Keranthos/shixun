...
func NewAdminHandler(adminService service.AdminService, ragHook *service.RagIndexHook) *AdminHandler {
	return &AdminHandler{adminService: adminService, ragHook: ragHook}
}
...
func (h *AdminHandler) ReviewItem(c *gin.Context) {
	...
	if h.ragHook != nil {
		...
		id := itemID
		if status == "approved" {
			h.ragHook.OnApproved(rt, id)
		} else if status == "rejected" {
			h.ragHook.OnRejected(rt, id)
		}
	}
