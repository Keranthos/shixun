...
func (r *projectRepository) GetByID(ctx context.Context, projectID string) (map[string]interface{}, error) {
	...
	var detail, contentInsight sql.NullString
	var createdAt time.Time
	
	// 只允许查看已审核通过的项目（pending和rejected的项目不应该公开显示）
	err := r.db.QueryRowContext(ctx, `
		SELECT project_id, resource_type, name, description,
		       CAST(content_insight AS CHAR) AS content_insight,
		       detail, github_url, category, cover, views, loves, collections, created_at
		FROM projects WHERE project_id = ? AND status = 'approved'`, projectID).Scan(
		&pID, &resourceType, &name, &description, &contentInsight, &detail,
		&githubURL, &category, &cover, &views, &loves, &collections, &createdAt)
	if err != nil {
