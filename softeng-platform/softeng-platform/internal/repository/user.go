	...
	for rows.Next() {
		...
		case "tool":
			...
			err := r.db.QueryRowContext(ctx,
				"SELECT resource_name, category, description, views, loves, status FROM tools WHERE resource_id = ?",
				resourceID,
			).Scan(&name, &category, &description, &views, &loves, &status)
			if err != nil || !status.Valid || status.String != "approved" || !name.Valid {
				continue
			}
			var collections int
		...
		case "course":
			...
			var views, loves int
			err := r.db.QueryRowContext(ctx,
				"SELECT name, semester, description, cover, views, loves FROM courses WHERE course_id = ?",
				resourceID,
			).Scan(&name, &semester, &description, &cover, &views, &loves)
	...
	case "tool":
		_, _ = r.db.ExecContext(ctx, `
			UPDATE tools SET collections = (
				...
				WHERE c.resource_type = 'tool' AND c.resource_id = tools.resource_id
