...
func (r *courseRepository) GetByID(ctx context.Context, courseID string) (map[string]interface{}, error) {
	...
	var description, contentInsight, learningPath sql.NullString
	err := r.db.QueryRowContext(ctx, `
		SELECT course_id, resource_type, name, semester, credit, cover, views, loves,
		COALESCE((SELECT COUNT(*) FROM collections col WHERE col.resource_type = 'course' AND col.resource_id = courses.course_id), 0) AS collections,
		COALESCE(description, '') AS description,
		CAST(content_insight AS CHAR) AS content_insight,
		CAST(learning_path AS CHAR) AS learning_path,
		created_at
		FROM courses WHERE course_id = ?`, courseID).Scan(
		&courseIDInt, &resourceType, &name, &semester, &credit, &cover, &views, &loves, &collections,
		&description, &contentInsight, &learningPath, &createdAt,
	)
	if err != nil {
	...
	return map[string]interface{}{
		...
		"contentInsight": parseJSONMap(contentInsight),
		"learningPath":   parseJSONMap(learningPath),
...
func (r *courseRepository) CollectCourse(ctx context.Context, userID int, courseID string) (map[string]interface{}, error) {
	...
	if exists > 0 {
		n, _ := r.getCourseCollectionCount(ctx, courseID)
		return map[string]interface{}{"iscollected": true, "collections": n}, nil
	}
	...
	r.syncCourseCollectionCount(ctx, courseID)
	n, _ := r.getCourseCollectionCount(ctx, courseID)
	return map[string]interface{}{"iscollected": true, "collections": n}, err
}
...
func (r *courseRepository) UncollectCourse(ctx context.Context, userID int, courseID string) (map[string]interface{}, error) {
	...
	if exists == 0 {
		n, _ := r.getCourseCollectionCount(ctx, courseID)
		return map[string]interface{}{"iscollected": false, "collections": n}, nil
	}
	...
	n, _ := r.getCourseCollectionCount(ctx, courseID)
	return map[string]interface{}{"iscollected": false, "collections": n}, err
}

func (r *courseRepository) getCourseCollectionCount(ctx context.Context, courseID string) (int, error) {
	var n int
	err := r.db.QueryRowContext(ctx,
		"SELECT COUNT(*) FROM collections WHERE resource_type = 'course' AND resource_id = ?",
		courseID).Scan(&n)
	return n, err
}

func (r *courseRepository) syncCourseCollectionCount(ctx context.Context, courseID string) {
	_, _ = r.db.ExecContext(ctx, `
		UPDATE courses SET collections = (
			SELECT COUNT(*) FROM collections c
			WHERE c.resource_type = 'course' AND c.resource_id = courses.course_id
