...
sequenceDiagram
  ...
  U->>FE: 输入问题（多轮 history）
  FE->>API: JSON { message, stream } + Bearer
  API->>S: Chat / ChatStream
  alt AGENT_BASE_URL 已配置
    S->>AG: 混合检索 Chroma+倒排
    AG->>AG: Gemini 流式生成
    AG-->>S: SSE delta + used[]
  else Agent 不可用
    S->>DB: LIKE 回退检索
    S-->>FE: fallbackReason + demo/LLM
  end
  S-->>FE: answer + sources[]
  FE-->>U: 流式展示 + 引用跳转
```
...
- **主路径**：Python Agent（8766）LangChain LCEL + Chroma 向量 + SQLite 倒排；Go 网关转发；支持 SSE 流式与 `[1][2]` 引用溯源。  
- **索引**：启动自动 reindex；全量重建先清空；审核通过 upsert、驳回 delete、资源更新 Sync。  
- **回退**：Agent 不可用时 LIKE + Gemini/OpenAI/demo，前端显示「已降级」。  
- **权限**：仅管理员；`RAG_RATE_LIMIT_PER_MIN` 限流。  
- 生成：优先 **Gemini**；失败时 `redactClientErr` 脱敏；前端 `postJSON` + SSE 流式。
...
**检索阶段（主路径）**：配置 `AGENT_BASE_URL` 后，Go 将请求转发至 Python LangChain Agent：对用户 query 做 Gemini embedding，在 Chroma 向量库与 SQLite 倒排索引间做混合检索与融合排序，返回工具/课程/项目 chunk；工具与项目限定 `approved`。

**检索阶段（回退）**：Agent 不可用时，`rag_agent.go` 回退 MySQL LIKE 多表检索，并返回 `fallbackReason` 供前端展示。

**生成阶段**：Agent 内 Gemini `generateContent`（支持 SSE 流式）；Go 回退路径同样优先 Gemini/OpenAI，皆无 Key 时 demo 模板。Prompt 要求“仅依据 Sources 回答”并标注 `[1][2]` 引用。

**索引同步**：`POST /api/agent/reindex` 全量重建（先 clear）；审核通过增量 upsert；驳回 delete；工具/项目更新后 SyncResource。

**安全与权限**：`agent.Use(AuthMiddleware(), AdminMiddleware(), AuthRateLimit)`；错误脱敏 `redactClientErr`。

**前端交互**：`RagAgent.vue` 默认 SSE 流式；Markdown 轻量渲染；点击 `[n]` 滚动至高亮引用；展示 Agent 状态与重建索引按钮。
...
已知限制：部分消息/邮件为演示逻辑；课程待审与工具/项目审核模型尚未完全统一；RAG 语料尚未纳入评论与课程资料附件。
...
**Q7：还有哪些已知限制？**  
A：站内消息/邮件部分为演示逻辑；课程审核状态与工具/项目未完全统一；RAG 语料可继续扩展（评论、资料）；详见第八章后续计划。
